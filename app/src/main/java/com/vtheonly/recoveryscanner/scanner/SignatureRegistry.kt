...
object SignatureRegistry {
    ...
    fun detect(input: InputStream, name: String): Detection? {
        ...
        fun startsWith(bytes: ByteArray): Boolean =
            bytes.isNotEmpty() && read >= bytes.size && header.copyOf(bytes.size).contentEquals(bytes)

        if (startsWith(byteArrayOf(0xFF.toByte(), 0xD8.toByte(), 0xFF.toByte())))
            return Detection(MediaType.JPEG, "jpg", 99)
        if (startsWith(byteArrayOf(0x89.toByte(), 0x50, 0x4E, 0x47)))
            return Detection(MediaType.PNG, "png", 99)
        if (startsWith("GIF87a".toByteArray()) || startsWith("GIF89a".toByteArray()))
            return Detection(MediaType.GIF, "gif", 99)
        if (read >= 12 && header.copyOfRange(0, 4).contentEquals("RIFF".toByteArray()) &&
            header.copyOfRange(8, 12).contentEquals("WEBP".toByteArray()))
            return Detection(MediaType.WEBP, "webp", 99)
        ...
        if (read >= 12 && header.copyOfRange(4, 8).contentEquals("ftyp".toByteArray())) {
            ...
            val type = when {
                brand in setOf("qt  ") -> MediaType.MOV
        ...
        // A content scan can also find embedded JPEG/PNG signatures in a non-media container.
        val jpeg = findMagic(header, byteArrayOf(0xFF.toByte(), 0xD8.toByte(), 0xFF.toByte()))
        if (jpeg > 0) return Detection(MediaType.JPEG, "jpg", 88, jpeg.toLong())
        val png = findMagic(header, byteArrayOf(0x89.toByte(), 0x50, 0x4E, 0x47))
        if (png > 0) return Detection(MediaType.PNG, "png", 88, png.toLong())
        return null
    }

    private fun findMagic(buffer: ByteArray, magic: ByteArray): Int {
        outer@ for (i in 1..buffer.size - magic.size) {
