...
class SamsungForensics(private val context: Context) {
    ...
    fun restoreName(file: File): String {
        ...
        return try { val exif = ExifInterface(file); val original = exif.getAttribute(ExifInterface.TAG_DATETIME_ORIGINAL) ?: exif.getAttribute(ExifInterface.TAG_DATETIME); if (original.isNullOrBlank()) file.name else "${original.replace(":", "").replace(" ", "_")}_${file.name}.${file.extension}" } catch (_: Throwable) { file.name }
    }
    ...
    fun scanUri(uri: Uri, name: String, directFile: File? = null): List<Embedded> {
        val size = directFile?.length() ?: querySize(uri); if (size <= 0) return emptyList()
        val start = maxOf(0L, size - 128L * 1024L * 1024L); val tail = readRange(uri, start, (size - start).toInt()) ?: return emptyList(); val results = mutableListOf<Embedded>()
        val seft = indexOf(tail, ascii("SEFT"), 0); val sefh = indexOf(tail, ascii("SEFH"), 0)
        if (seft >= 0) results += parseSef(uri, name, tail, start, size, sefh)
        var search = 0
        while (search + 8 <= tail.size) {
            val ftyp = indexOf(tail, ascii("ftyp"), search); if (ftyp < 4) break
            val localStart = ftyp - 4; val absolute = start + localStart
            val boundary = when { sefh > localStart -> start + sefh; seft > localStart -> start + seft; else -> size }
            val length = boundary - absolute; val header = readRange(uri, absolute, 64) ?: ByteArray(0); val detected = SignatureRegistry.detect(header.inputStream(), "embedded.mp4")?.type
            if (length > 1024 && detected in setOf(MediaType.MP4, MediaType.MOV, MediaType.THREE_GP) && results.none { it.offset == absolute && it.length == length }) results += Embedded(uri, name, MediaType.MP4, absolute, length, if (seft >= 0) "MotionPhoto_Data / SEFT fallback" else "Embedded ISO-BMFF", RecoveryQuality.LIKELY_RECOVERABLE)
            search = ftyp + 4
        }
    ...
    private fun parseSef(uri: Uri, name: String, tail: ByteArray, globalStart: Long, fileSize: Long, sefhLocal: Int): List<Embedded> {
        ...
        // Real Samsung SEFH records use little-endian version/count and little-endian offset/length.
        if (sefh >= 0 && sefhLocal + 12 <= tail.size) {
            val version = le32(tail, sefhLocal + 4); val count = le32(tail, sefhLocal + 8)
            if (count in 1..4096 && version in 1..10000) {
                var p = sefhLocal + 12
                repeat(count.toInt()) {
                    if (p + 12 > tail.size) return@repeat
                    val marker = le16(tail, p + 2); val offsetBack = le32(tail, p + 4); val length = le32(tail, p + 8); val absolute = sefh - offsetBack
                    if (absolute >= 0 && length > 16 && absolute + length <= fileSize) {
                        val type = detectTypeAt(uri, absolute); val key = if (marker == 0x0A30L) "MotionPhoto_Data" else "SEF field 0x${marker.toString(16)}"
                        if (type != MediaType.UNKNOWN || marker == 0x0A30L) result += Embedded(uri, name, if (type == MediaType.UNKNOWN) MediaType.MP4 else type, absolute, length, key, RecoveryQuality.VALID)
                    }
                    p += 12
                }
            }
        }
        // Named trailer records catch auxiliary fields whose numeric markers are not stable/documented.
        for (key in listOf("MotionPhoto_Data", "DualShot_ExtraImage", "DualShot_DepthMap")) {
            var at = 0
            while (true) {
                val found = indexOf(tail, ascii(key), at); if (found < 0) break
                val candidate = nearestPayload(uri, tail, globalStart, fileSize, sefhLocal, found)
                if (candidate != null) result += Embedded(uri, name, candidate.first, candidate.second, candidate.third, key, RecoveryQuality.VALID)
                at = found + key.length
            }
        }
    ...
    private fun nearestPayload(uri: Uri, tail: ByteArray, globalStart: Long, fileSize: Long, sefhLocal: Int, keyPos: Int): Triple<MediaType, Long, Long>? {
        ...
        if (nameSize > 0 && nameSize <= 1024) {
            val payloadStart = globalStart + keyPos + nameSize; val end = if (sefhLocal >= 0) globalStart + sefhLocal else fileSize
            if (payloadStart >= 0 && end > payloadStart && end <= fileSize) { val type = detectTypeAt(uri, payloadStart); if (type != MediaType.UNKNOWN) return Triple(type, payloadStart, end - payloadStart) }
        }
        ...
        val jpeg = indexOf(tail, byteArrayOf(0xFF.toByte(), 0xD8.toByte(), 0xFF.toByte()), keyPos); if (jpeg >= 0) { val payloadStart = globalStart + jpeg; val end = if (sefhLocal >= 0) globalStart + sefhLocal else fileSize; if (end > payloadStart) return Triple(MediaType.JPEG, payloadStart, end - payloadStart) }
        return null
    }
