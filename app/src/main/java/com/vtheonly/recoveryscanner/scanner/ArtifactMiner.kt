...
class ArtifactMiner(private val context: Context) {
    ...
    private fun walk(dir:DocumentFile,out:MutableList<Artifact>,progress:(String)->Unit){for(file in dir.listFiles()){if(file.isDirectory){walk(file,out,progress);continue};val name=file.name?:"unnamed";progress(name);val detection=context.contentResolver.openInputStream(file.uri)?.use{SignatureRegistry.detect(it,name)};if(detection!=null){out+=Artifact(file.uri,name,detection.type,detection.offset,file.length(),"filesystem");if(detection.type==MediaType.JPEG||detection.type==MediaType.HEIC){extractExifThumbnail(file.uri)?.let{bytes->if(bytes.size>2048){val f=saveTemp("exif-$name",bytes);out+=Artifact(Uri.fromFile(f),"$name — EXIF preview.jpg",MediaType.JPEG,0,bytes.size.toLong(),"EXIF embedded thumbnail",RecoveryQuality.VALID)}}};if(detection.type==MediaType.MP4||detection.type==MediaType.MOV||detection.type==MediaType.THREE_GP){extractVideoFrame(file.uri)?.let{bytes->if(bytes.size>2048){val f=saveTemp("video-frame-$name",bytes);out+=Artifact(Uri.fromFile(f),"$name — keyframe.jpg",MediaType.JPEG,0,bytes.size.toLong(),"video keyframe preview",RecoveryQuality.VALID)}}}};if(name.contains("thumbdata",true))out+=carveJpegs(file.uri,name,"thumbdata")}}
    private fun saveTemp(prefix:String,bytes:ByteArray):File{val file=File.createTempFile(prefix.take(24),".jpg",context.cacheDir);file.outputStream().use{it.write(bytes)};return file}
