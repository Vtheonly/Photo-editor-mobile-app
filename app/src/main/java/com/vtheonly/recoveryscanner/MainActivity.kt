...
class MainActivity : Activity() {
    ...
    private fun artifactMine(uri: Uri) {
        ...
        scope.launch(Dispatchers.IO) {
            val miner = ArtifactMiner(this@MainActivity)
            val samsung = SamsungForensics(this@MainActivity)
            val converted = mutableListOf<ScanResult>()
            fun addArtifact(a: ArtifactMiner.Artifact) {
                val ext = a.type.extension(); val originalExt = a.displayName.substringAfterLast('.', "")
                converted += ScanResult(a.displayName, a.uri.toString(), a.type, ext, originalExt, a.length, 99, originalExt.lowercase() != ext, a.offset > 0, a.offset, a.length, a.quality)
            }
            for (a in miner.scanTree(uri) { }) addArtifact(a)
            if (DirectStorageScanner(this@MainActivity).isAvailable()) {
                for (file in DirectStorageScanner(this@MainActivity).scan { }) {
                    ...
                    if (file.extension.lowercase() in setOf("jpg", "jpeg", "heic", "heif")) {
                        for (e in samsung.scanFile(file)) converted += ScanResult("$restoredName — ${e.key}", e.sourceUri.toString(), e.type, e.type.extension(), file.extension, e.length, 100, true, true, e.offset, e.length, e.quality)
                    }
                }
            }
