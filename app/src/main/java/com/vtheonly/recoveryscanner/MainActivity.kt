...
class MainActivity : Activity() {
    ...
    private lateinit var status:TextView;private lateinit var resultContainer:LinearLayout;private lateinit var access:Button;private lateinit var chooseFolder:Button;private lateinit var deepScan:Button;private lateinit var artifactScan:Button;private lateinit var scanMedia:Button;private lateinit var recover:Button
    ...
    private fun buildUi(){val root=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;setPadding(32,28,32,24)};root.addView(TextView(this).apply{text="Recovery Scanner";textSize=28f},match());root.addView(TextView(this).apply{text="Forensic-style no-root media recovery over storage the user explicitly grants to the app.";textSize=14f;setPadding(0,8,0,20)},match());access=Button(this).apply{setOnClickListener{requestAllFilesAccess()}};root.addView(access,match());chooseFolder=Button(this).apply{text="Normal scan: selected folder";setOnClickListener{chooseScanFolder(false)}};deepScan=Button(this).apply{text="Maximum scan: every accessible byte";setOnClickListener{chooseScanFolder(true)}};artifactScan=Button(this).apply{text="Artifact mine: thumbnails + hidden media + Samsung";setOnClickListener{chooseScanFolder(ACTION_ARTIFACTS)}};scanMedia=Button(this).apply{text="MediaStore: normal + trash + pending";setOnClickListener{requestMediaPermissionAndScan()}};recover=Button(this).apply{text="Recover selected";isEnabled=false;setOnClickListener{chooseRecoveryFolder()}};root.addView(chooseFolder,match());root.addView(deepScan,match());root.addView(artifactScan,match());root.addView(scanMedia,match());root.addView(recover,match());status=TextView(this).apply{text="The scanner never reads deleted physical UFS/eMMC blocks; it mines every user-accessible source instead.";textSize=14f;setPadding(0,18,0,14)};root.addView(status,match());val scroll=ScrollView(this);resultContainer=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL};scroll.addView(resultContainer);root.addView(scroll,LinearLayout.LayoutParams(-1,0,1f));setContentView(root);updateAccessButton()}
    ...
    private fun artifactMine(uri:Uri){setBusy(true,"Mining hidden folders, thumbdata and Samsung embedded media...");scope.launch(Dispatchers.IO){val miner=ArtifactMiner(this@MainActivity);val artifacts=miner.scanTree(uri){name->};val samsung=SamsungArtifactScanner(this@MainActivity);val converted=mutableListOf<ScanResult>();for(a in artifacts){converted+=ScanResult(a.displayName,a.uri.toString(),a.type,a.type.extension(),a.displayName.substringAfterLast('.',""),a.length,99,a.displayName.substringAfterLast('.',"").lowercase()!=a.type.extension(),a.offset>0,a.offset,a.length,a.quality);if(a.type==MediaType.JPEG||a.type==MediaType.HEIC){for(e in samsung.scan(a.uri,a.displayName)){converted+=ScanResult("${a.displayName}-${e.key}",e.sourceUri.toString(),e.type,"mp4",a.displayName.substringAfterLast('.',""),e.length,94,true,true,e.offset,e.length,e.quality)}}};if(DirectStorageScanner(this@MainActivity).isAvailable()){for(file in DirectStorageScanner(this@MainActivity).scan{}){val detected=FileInputStream(file).use{com.vtheonly.recoveryscanner.scanner.SignatureRegistry.detect(it,file.name)};if(detected!=null)converted+=ScanResult(file.name,Uri.fromFile(file).toString(),detected.type,detected.extension,file.extension, file.length(),detected.confidence,file.extension.lowercase()!=detected.extension,detected.offset>0,detected.offset,file.length(),RecoveryQuality.LIKELY_RECOVERABLE)}};withContext(Dispatchers.Main){results=converted.distinctBy{"${it.sourceUri}:${it.offset}:${it.detectedType}"};renderResults();setBusy(false,"Artifact mining complete: ${results.size} candidates.")}}}
    ...
    private fun MediaType.extension()=when(this){MediaType.JPEG->"jpg";MediaType.PNG->"png";MediaType.GIF->"gif";MediaType.WEBP->"webp";MediaType.BMP->"bmp";MediaType.HEIC->"heic";MediaType.MP4->"mp4";MediaType.MOV->"mov";MediaType.THREE_GP->"3gp";MediaType.UNKNOWN->"bin"}
