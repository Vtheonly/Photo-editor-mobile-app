...
import com.vtheonly.recoveryscanner.scanner.ShizukuCacheScanner
...
class MainActivity : Activity() {
    ...
    private lateinit var status: TextView; private lateinit var resultContainer: LinearLayout; private lateinit var access: Button; private lateinit var shizuku: Button; private lateinit var chooseFolder: Button; private lateinit var deepScan: Button; private lateinit var artifactScan: Button; private lateinit var scanMedia: Button; private lateinit var recover: Button
    private val selected = linkedMapOf<String, ScanResult>(); private var results = emptyList<ScanResult>(); private var pendingFolderAction = ACTION_SCAN_FOLDER
    ...
    private fun buildUi() {
        ...
        shizuku = Button(this).apply { setOnClickListener { val s = ShizukuAccess(); when { !s.available() -> Toast.makeText(this@MainActivity, "Install and start Shizuku first.", Toast.LENGTH_LONG).show(); !s.granted() -> s.requestPermission(); else -> scanShizukuCaches() }; updateShizukuButton() } }
        ...
        status = TextView(this).apply { text = "The scanner does not claim access to deleted physical UFS/eMMC blocks. It mines user-accessible filesystem, cache, trash, container and indexed artifacts."; textSize = 14f; setPadding(0, 18, 0, 14) }; root.addView(status, match())
    ...
    private fun updateShizukuButton() { val s = ShizukuAccess(); shizuku.text = when { !s.available() -> "Shizuku: not running"; s.granted() -> "Shizuku: AUTHORIZED — scan Samsung/app caches"; else -> "Shizuku: running — authorize scanner" } }
    private fun scanShizukuCaches() { setBusy(true, "Shizuku: scanning shell-readable Samsung/app caches..."); ShizukuCacheScanner { paths -> scope.launch(Dispatchers.IO) { val miner = ArtifactMiner(this@MainActivity); val found = mutableListOf<ScanResult>(); for (path in paths) for (a in miner.scanFile(File(path))) { val ext = a.type.extension(); found += ScanResult(a.displayName, a.uri.toString(), a.type, ext, a.displayName.substringAfterLast('.', ""), a.length, 99, false, a.offset > 0, a.offset, a.length, a.quality) }; withContext(Dispatchers.Main) { results = found.distinctBy { "${it.sourceUri}:${it.offset}:${it.recoveredLength}:${it.detectedType}" }; renderResults(); setBusy(false, "Shizuku cache scan complete: ${results.size} candidates.") } } }.start() }
    ...
    private fun chooseScanFolder(mode: Boolean) { pendingFolderAction = if (mode) ACTION_DEEP_SCAN else ACTION_SCAN_FOLDER; pickTree() }; private fun chooseScanFolder(action: Int) { pendingFolderAction = action; pickTree() }; private fun pickTree() { startActivityForResult(Intent(Intent.ACTION_OPEN_DOCUMENT_TREE).apply { addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION or Intent.FLAG_GRANT_PERSISTABLE_URI_PERMISSION) }, REQUEST_TREE) }; private fun chooseRecoveryFolder() { pendingFolderAction = ACTION_RECOVER; startActivityForResult(Intent(Intent.ACTION_OPEN_DOCUMENT_TREE).apply { addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION or Intent.FLAG_GRANT_WRITE_URI_PERMISSION or Intent.FLAG_GRANT_PERSISTABLE_URI_PERMISSION) }, REQUEST_TREE) }
    ...
    private fun artifactMine(uri: Uri) { setBusy(true, "Samsung forensic pipeline: filesystem -> hidden/trash -> signatures -> thumbdata -> EXIF -> keyframes -> SEF -> archives..."); scope.launch(Dispatchers.IO) { val miner = ArtifactMiner(this@MainActivity); val samsung = SamsungForensics(this@MainActivity); val converted = mutableListOf<ScanResult>(); fun addArtifact(a: ArtifactMiner.Artifact) { val ext = a.type.extension(); val originalExt = a.displayName.substringAfterLast('.', ""); converted += ScanResult(a.displayName, a.uri.toString(), a.type, ext, originalExt, a.length, 99, originalExt.lowercase() != ext, a.offset > 0, a.offset, a.length, a.quality) }; for (a in miner.scanTree(uri) { }) addArtifact(a); if (DirectStorageScanner(this@MainActivity).isAvailable()) for (file in DirectStorageScanner(this@MainActivity).scan { }) { val restoredName = samsung.restoreName(file); for (a in miner.scanFile(file)) addArtifact(if (a.displayName == file.name) a.copy(displayName = restoredName) else a); if (file.extension.lowercase() in setOf("jpg", "jpeg", "heic", "heif")) for (e in samsung.scanFile(file)) converted += ScanResult("$restoredName — ${e.key}", e.sourceUri.toString(), e.type, e.type.extension(), file.extension, e.length, 100, true, true, e.offset, e.length, e.quality) }; withContext(Dispatchers.Main) { results = converted.distinctBy { "${it.sourceUri}:${it.offset}:${it.recoveredLength}:${it.detectedType}:${it.sourceName}" }; renderResults(); setBusy(false, "Samsung forensic scan complete: ${results.size} candidates. Included hidden/dot folders, Samsung trash heuristics, .thumbdata, EXIF previews, video keyframes, SEF/Motion Photo/DualShot, Samsung Notes/Smart Switch archives and direct shared storage.") } } }
    ...
    private fun match() = LinearLayout.LayoutParams(-1, ViewGroup.LayoutParams.WRAP_CONTENT); private fun matchHeight() = LinearLayout.LayoutParams(-1, 1); override fun onDestroy() { scope.cancel(); super.onDestroy() }
